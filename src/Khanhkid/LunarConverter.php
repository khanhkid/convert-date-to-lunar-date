...
class LunarConverter
{
    ...
    public static function LunarDateToEnglishDate($lunar)
    {
        $days = self::$lunar_month_days[$lunar->LYear - self::$lunar_month_days[0]];
        $leap = self::GetBitInt($days, 4, 13);
        $offset = 0;
        $loopend = $leap;
        if (!$lunar->isleap) {
            if ($lunar->LMonth <= $leap || $leap == 0) {
        ...
        $solar11 = self::$solar_1_1[$lunar->LYear - self::$solar_1_1[0]];
    ...
    public static function EnglishDateToLunarDate($solar)
    {
        $lunar = new LunarDate();
        $index = $solar->EYear - self::$solar_1_1[0];
        $data = ($solar->EYear << 9) | ($solar->EMonth << 5) | ($solar->EDay);
        ...
        $m = self::GetBitInt($solar11, 4, 5);
        $d = self::GetBitInt($solar11, 5, 0);
        $offset = self::SolarToInt($solar->EYear, $solar->EMonth, $solar->EDay) - self::SolarToInt($y, $m, $d);
